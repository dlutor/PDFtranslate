...
var PDFViewerApplication = {
  ...
  run: function run(config) {
    this.initialize(config).then(webViewerInitialized).then(function(){
      //add by huangkefen ,auto open a window in firefox 60 or upper
      if (document.readyState === 'interactive' || document.readyState === 'complete') {
        // 查询配置信息，并决定是否自动加载取词脚本以及自动弹出选择文件的窗口
        let callback  = function (data) {
          const {AppConfig}=data;
          if(AppConfig&&AppConfig.publicConfig){
            const autoPopup = AppConfig.publicConfig.autoPopup;
            if(autoPopup){
              document.getElementById(PDFViewerApplication.appConfig.openFileInputName).click();
            }
            const autoLoad = AppConfig.publicConfig.autoLoadWhenOpenPDF ;
            if(autoLoad){
              let polyfill =document.createElement("script");
              polyfill.setAttribute("src","../../../resources/browser-polyfill.min.js");
              document.body.appendChild(polyfill);
              let jqueryScript =document.createElement("script");
              jqueryScript.setAttribute("src","../../../resources/js/jquery.min.js");
              document.body.appendChild(jqueryScript);
              let icibaScript =document.createElement("script");
              icibaScript.setAttribute("src","../../translate-iciba.js");
              document.body.appendChild(icibaScript);
              let icibaCss = document.createElement("link");
              icibaCss.setAttribute("rel","stylesheet");
              icibaCss.setAttribute("href","../../../content_css/huaci_mini.css");
              document.body.appendChild(icibaCss);
        ...
        if(isChrome){
          chrome.storage.local.get("AppConfig",callback);
        }else {
  ...
  load: function load(pdfDocument) {
    ...
    pdfDocument.getMetadata().then(function (_ref12) {
      ...
      if(PDFViewerApplication.currentFile && PDFViewerApplication.currentFile.name){
        _this5.setTitle(PDFViewerApplication.currentFile.name);
      }
    });
  },
...
var webViewerFileInputChange = void 0;
{
  webViewerFileInputChange = function webViewerFileInputChange(evt) {
    ...
    PDFViewerApplication.currentFile = file ;
