	...
	if ("undefined" == typeof browser || Object.getPrototypeOf(browser) !== Object.prototype) {
		a.exports = (e => {
			...
			class g extends WeakMap {
				...
				get(v) {
					return this.has(v) || this.set(v, this.createItem(v)),
					super.get(v)
				}
			}
			const h = v => {
			...
			i = (v, w) => {
				return (...x) => {
					var t=e.runtime.lastError ;
					//? 
					//var t=v.reject(e.runtime.lastError);
					//: 
					var t=w.singleCallbackArg ;
					//|| 
					var t=1 >= x.length ? v.resolve(x[0]) : v.resolve(x);
				}
			},
					...
					return new Promise((A, B) => {
						if (w.fallbackToNoCallback)
							try {
								...
								console.warn(`${v} API method doesn't seem to support the callback parameter, ` + "falling back to call it without a callback: ", C),
								y[v](...z),
			...
			const q = new g(v => {
					return "function" == typeof v ? function (x, y, z) {
						...
						C = new Promise(G => {
								B = function (H) {
									p || (console.warn("Returning a Promise is the preferred way to send a reply from an onMessage/onMessageExternal listener, as the sendResponse will be removed from the specs (See https://developer.mozilla.org/docs/Mozilla/Add-ons/WebExtensions/API/runtime/onMessage)", new Error().stack), p = !0),
									A = !0,
									G(H)
								}
							});
						...
						const F = G => {
							G.then(H => {
								...
								let I;
								I = H && (H instanceof Error || "string" == typeof H.message) ? H.message : "An unexpected error occurred",
								z({
